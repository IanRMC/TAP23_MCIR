...
namespace Vista
{
    public partial class App : Form
    {
        ...
        public static Boolean huboCambio = false;
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            if (dgvProduct.SelectedRows.Count > 0)
            {
                int index = dgvProduct.CurrentRow.Index;
                Product product = products[index];
                DialogResult d = MessageBox.Show("Estas seguro de que deseas eliminar " + product.ProductName, "SI O NO ", MessageBoxButtons.YesNo);
                if (d == DialogResult.Yes)
                {
                    ProductDAO productsDAO = new ProductDAO();
                    int codigo = new ProductDAO().Delete(product.ProductID);
                    ...
                    {
                        MessageBox.Show("No se a podido eliminar el producto " + nombre);
                    }
                    ActualizarTabla();
                }
                else 
